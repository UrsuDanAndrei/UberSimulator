	...
	public:
		...
		void task1_solver(std::ifstream& fin, std::ofstream& fout) {
			...
			for (int i = 1; i <= q1; ++i) {
				...
				fin >> junction1 >> junction2;

				// daca distanta este -1 inseamna ca nu exista drum junction1-2
		...
		void task2_solver(std::ifstream& fin, std::ofstream& fout) {
			...
			for (int i = 1; i <= q2; ++i) {
				...
				// se afiseaza distanta de la junction1 la junction2
				fout << graph.getDist(junction1, junction2) << std::endl;
			}
		}
		...
		void task5_solver(std::ifstream& fin, std::ofstream& fout) {
			...
			graph.displayAllNeighbours(disp,
